...
function mod:OnEngage()
	...
	self:CDBar("priestess_adds", 26, L["priestess_adds_message"], L.priestess_adds_icon)
	self:CDBar(-7062, 8) -- Quicksand
	self:Bar(136990, 17) -- Frostbite -- might be 7.5?
	self:Bar(-7054, 30) -- Frigid Assault
...
function mod:RecklessCharge(unit, _, _, _, spellId)
	if spellId == 137107 and UnitBuff(unit, self:SpellName(136442)) then
		self:Bar(137122, 26) -- Show timer when possessed
	end
end
	...
	function mod:PossessedApplied(args)
		...
		if mobId == 69131 then -- Frost King
			...
			if bitingColdStart then
				self:CDBar(136990, 44 - (GetTime() - bitingColdStart)) -- Frostbite -- CD bar because of Possessed buff travel time
			end
		end
	end
	...
	function mod:PossessedRemoved(args)
		...
		if mobId == 69131 then -- Frost King
			...
			if frostBiteStart then
				self:Bar(136992, 44 - (GetTime() - frostBiteStart)) -- Biting Cold
			end
		end
	end
end
