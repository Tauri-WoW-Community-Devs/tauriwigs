...
function mod:GetOptions()
	return {
		...
		{144005, "FLASH", "SAY"}, {143990, "FLASH", "SAY", "ICON"}, 143973, -- Wavebinder Kardris
...
function mod:FroststormStrike(args)
	...
	if UnitDetailedThreatSituation("player", boss) then -- if tanking Haromm
...
function mod:FallingAsh(args)
	self:Message(args.spellId, "Attention", nil, CL.cast:format(CL.count:format(args.spellName, ashCounter)))
	self:ScheduleTimer("Message", 14.2, args.spellId, "Attention", self:Healer() and "Info", CL.soon:format(CL.count:format(args.spellName, ashCounter)))
	ashCounter = ashCounter + 1
	self:Bar(args.spellId, 16.2, CL.count:format(args.spellName, ashCounter))
end

do
	local function blobsWarning(spellId)
		mod:Say(spellId, "{cross}", true)
	end
	function mod:FoulGeyser(args) -- Blobs
		...
		if self:Me(args.destGUID) then
			...
			self:Say(args.spellId, L.blobs)
			if self:Heroic() then
				self:ScheduleTimer(blobsWarning, 2, args.spellId)
				self:ScheduleTimer(blobsWarning, 4, args.spellId)
				self:ScheduleTimer(blobsWarning, 6, args.spellId)
				self:ScheduleTimer(blobsWarning, 8, args.spellId)
			end
