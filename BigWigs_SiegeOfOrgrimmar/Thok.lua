...
function mod:GetOptions()
	return {
		148145, "adds",
		{"tank_debuffs", "TANK_HEALER"}, -7963, 143428, 143777, 143783, -- stage 1
...
do
	...
	function mod:BloodFrenzyOver(args)
		...
		self:CDBar(-7963, self:LFR() and 18 or 13.3) -- Deafening Screech
...
function mod:TankDebuffCasts(_, spellName, _, _, spellId)
	if spellId == 143426 then -- Fearsome Roar
		self:CDBar("tank_debuffs", 11, spellName, spellId)
	elseif spellId == 143780 then -- Acid Breath
		self:CDBar("tank_debuffs", 13.2, spellName, spellId)
	elseif spellId == 143773 then -- Freezing Breath
		self:CDBar("tank_debuffs", 6.1, spellName, spellId)
	elseif spellId == 143767 then -- Scorching Breath
		self:CDBar("tank_debuffs", 10.8, spellName, spellId)
	end
end
